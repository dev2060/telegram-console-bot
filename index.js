...
const telegramToken = require('./bot_token');
...
function processCommands(message) {
  if (message) {
    ...
    console.log("[Hello, processCommands1]");
    return commands[command](commandArguments[command]);
  }

  console.log("[Hello, processCommands2]");
...
exports.handler = (event, context) => {
  ...
  if (chatId) {
    processCommand.then((response) => {
      console.log("[sendMessageToTelegram_main1]");
      const processTelegram = sendMessageToTelegram(chatId, response);
      processTelegram.then(() => {
        console.log("[sendMessageToTelegram1a]");
        context.succeed();
      }).catch(() => {
        console.log("[sendMessageToTelegram1b]");
        context.fail();
      });
    }).catch((error) => {
      console.log("[sendMessageToTelegram2]");
